...
use simulation::monte_carlo::{MonteCarloResult, run_monte_carlo};
...
pub fn run_monte_carlo_wasm(
    ...
    let policy: WithdrawalPolicy = serde_wasm_bindgen::from_value(policy_js)
        .map_err(|e| JsValue::from_str(&format!("Invalid policy: {}", e)))?;

    let params: SimulationParams = serde_wasm_bindgen::from_value(params_js)
        .map_err(|e| JsValue::from_str(&format!("Invalid simulation params: {}", e)))?;

    let points: Vec<models::ModelPoint> = serde_wasm_bindgen::from_value(points_js)
        .map_err(|e| JsValue::from_str(&format!("Invalid price projection: {}", e)))?;

    let result: MonteCarloResult = run_monte_carlo(&policy, &params, &points, seed, None)
        .map_err(|e| JsValue::from_str(&e))?;
...
mod tests {
    ...
    #[wasm_bindgen_test]
    fn run_monte_carlo_wasm_returns_summary() {
        ...
        let result = run_monte_carlo_wasm(policy_js, params_js, prices_js, 42).unwrap();
        let mc: MonteCarloResult = serde_wasm_bindgen::from_value(result).unwrap();
        assert_eq!(mc.run_count, 10_000);
        assert!((mc.summary.success_pct - 100.0).abs() < 1e-9);
        assert_eq!(mc.percentiles.len(), 50);
    }
}
