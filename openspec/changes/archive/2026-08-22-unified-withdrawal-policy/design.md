...
9. **UI: collapsible tabbed card below the chart.** A `Tabs` control (Base UI) in the main column: Price model (`ModelSelector` with per-model controls), Scenario (current `ParameterPanel` + new inflation field + the projection horizon), Withdrawal (preset cards + visible knobs). The card is collapsible via a header toggle (44px target); the active tab survives collapse/expand. The desktop sidebar is removed; mobile and desktop share the same structure (chart first, controls below, results below the controls). Rationale: one layout for all viewports, chart-first on mobile per the mission. Preset selection renders as a row of cards; editing any knob marks the preset label dirty (asterisk) without switching to Custom. Knob visibility rules: inflation hidden for %-of-current; guardrail knobs only when guardrails on; valuation knobs only when valuation on; buffer refill knobs only when buffer on.
...
12. **Band-path simulation per model.** The plan simulates the policy against one price model's **band paths** — median, ±1σ, ±2σ, and percentile bands (P10/P25/P75/P90) where the model emits them — as separate deterministic runs. The path price feeds both the withdrawal math and the phase indicator (quantile of the path price inside the model's residual distribution), so bear/fair/euphoria phases are real today, not only under Phase 9 Monte Carlo. `ModelPoint` gains an optional `pathPriceUsd`; absent it defaults to the median, keeping the median-path contract. To make euphoria reachable on every model at the default thresholds (85th percentile), S2F and Bitcoin24 additionally emit 2σ bands (Power Law already supports 1σ/2σ/custom percentiles). A run covers the retirement years spanned by the model projection; the UI reports truncation when the projection ends early. This is deterministic band enumeration, not Monte Carlo: no randomness, no sampled-path percentile summaries.

13. **One model drives the plan.** Guardrail triggers and buffer phases only make sense against a single model's price signal, so the plan results carry a labeled "Price model used" selector among the *visible* models (defaulting to the first visible, falling back when the selected model is hidden). The chart may still overlay multiple models — overlays are visualization; the plan is a simulation. The path selector then picks which band path's year-by-year results are shown. Path tiles use directional names — Medium, Bearish, Bullish (Deep bear / Deep bull for 2σ) — with the band descriptor (fair, −1σ, +1σ, …) on the second line and the outcome summary (end-of-plan BTC or depletion year, final phase) on the third.
...
- [Band-path runs multiply engine cost] → paths are few (≤5 per model); each run is a cheap Rust loop; JS holds results vectors only. Fan charts stay Phase 10.
- [Euphoria unreachable on S2F/CAGR] → both models emit 2σ bands; 2σ-high ≈ 98th percentile → euphoria at default thresholds. Model tests updated alongside.
